...
public class ScrambleList {
    ...
    public ScrambleList(RecipeList recipesIn,ScrambleBook book) {
        recipes = recipesIn.getRecipes();
        this.singleResultItem =recipesIn.hasSingleResultItem();

        this.updateKnownRecipes(book);
    }

    public static List<ScrambleList> fromRecipeList(List<RecipeList> recipeLists, ScrambleBook book) {
    ...
    public void updateKnownRecipes(ScrambleBook book) {
    ...
    public void canCraft(RecipeItemHelper handler, int width, int height, ScrambleBook book) {
        for(int i = 0; i < this.recipes.size(); ++i) {
    ...
    //converts from ModRecipes to ScrambleLists
    public static List<ScrambleList> loadAllRecipes(UUID player, ResourceLocation craftingBlock)
    {
        List<ScrambleList> all = new ArrayList<>();
        for (ModRecipe recipe: ModRecipeRegistry.getRecipeList(player,craftingBlock))
        {
            ScrambleList scramble =new ScrambleList();
            scramble.add(recipe);
            all.add(scramble);
        }
        return all;
